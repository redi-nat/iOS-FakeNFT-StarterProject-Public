...
final class UsersServiceImpl: UsersService {
    ...
    func loadUsers(page: Int = 0, size: Int = 20, completion: @escaping (Result<[User], Error>) -> Void) {
        ...
        AppLogger.debug("📡 Отправляем запрос на загрузку пользователей...", category: .network)
        
        networkClient.send(request: request, type: [User].self) { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let users):
                    AppLogger.info("✅ Успешно загружено \(users.count) пользователей", category: .network)
                    completion(.success(users))
                case .failure(let error):
                    AppLogger.error("❌ Ошибка загрузки: \(error.localizedDescription)", category: .network)
